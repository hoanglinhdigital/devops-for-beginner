...
#Modify code, push lên Github.
#Chạy lại job của CodeBuild, kiểm tra ECR được tạo ra.

#Truy cập vào Task Definition, kiểm tra có một revision mới được tạo ra ứng với ECR image vừa build xong.
...
#Truy cập ALB thông qua port 80 để kiểm tra version đã được switch sang OK.
...
- Thêm file appspec.yml vào artiract output để CodeDeploy có thể deploy tự động trong luồng CICD của CodePipeline.
    ...
    - appspec.yaml: copy nguyên từ Github repo trong quá trình build.
